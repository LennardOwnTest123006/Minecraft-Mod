...
3. Drop `starforge-1.0.0.jar` into the same `mods` folder.
...
progression stays gated behind the Wither.
...
* An 11-step advancement tree on its own tab, with a custom background.
...
references against it, plus every texture and model reference on disk.
